...
for row in myReader:
    ...
    store2lat = row[4]
    store2lon = row[5]
    tempDist.append((store1, store2, store1lat, store1lon, store2lat, store2lon, distance, driving_time))
    
sqlStringInsert = "INSERT INTO mcdinfo VALUES (?,?,?,?,?,?,?,?)"
myCursor.executemany(sqlStringInsert, tempDist)
myConnection.commit()
tempDist = []

    #myCursor.execute('INSERT INTO mcdInfo VALUES(?,?,?,?,?,?,?,?);', row[0], row[1], row[2], row[3], row[4], row[5], distance, driving_time)
