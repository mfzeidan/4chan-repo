...
for row in myReader:
    ...
    haversineRow.append((row[0], row[1], row[2], row[3], row[4], row[5], distance))
...
myConnection.commit()   
    ...
    #UpdateSQLString = """UPDATE mcdonalds_time SET Haversine = distance 
...
#    latlon1 = lat1+ ',' +lon1 
#    latlon2 = lat2+ ',' + lon2   
#    originial_coordinates = latlon1
#    destination_coordinates = latlon2
#    url = 'http://maps.googleapis.com/maps/api/distancematrix/json?origins=%s&destinations=%s&mode=driving&language=en-EN&sensor=false'%(str(originial_coordinates),str(destination_coordinates))
#    result= json.load(urllib.urlopen(url))
#    
