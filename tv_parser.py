...
while True:
	...
	try:
		for x in str_thread_ids:
			api_4chan = 'https://a.4cdn.org/tv/thread/%s.json' % (x)
			link_4chan = 'http://boards.4chan.org/tv/thread/%s' % (x)
			print api_4chan
			print link_4chan
	...
	sql_statement = """UPDATE json_carry set json = %s WHERE board = 'tv'"""

	try:
		cur.execute("""UPDATE json_carry set json = '%s' WHERE board = 'tv'""" % lists) 
		con.commit()
		print "sql updated!"
	except:
		con.rollback()
