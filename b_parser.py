...
while True:
	...
	try:
		for x in str_thread_ids:
			api_4chan = 'https://a.4cdn.org/b/thread/%s.json' % (x)
			link_4chan = 'http://boards.4chan.org/b/thread/%s' % (x)
			print api_4chan
			print link_4chan
	...
	sql_statement = """UPDATE json_carry set json = %s WHERE board = 'b'"""

	try:
		cur.execute("""UPDATE json_carry set json = '%s' WHERE board = 'b'""" % lists) 
		con.commit()
		print "sql updated!"
	except:
		con.rollback()
